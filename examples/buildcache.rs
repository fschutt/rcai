...
impl SemanticAnalyzer {
                ...
                for sentence in sentences {
                    ...
                    for token in content_words {
                        ...
                        if let Some(concept) = self.get_concept_mut(word_id) {
                            println!("adding relationship of word {word} to glossary entry: {concept:?}");
                            let relation_set = concept.relations
    ...
    pub fn save_to_file(&self, path: &str) -> Result<(), String> {
        ...
        pub fn add_word(&mut self, word: &str, wordnet: &WordNetIntegration) -> ConceptId {
            ...
            let existing_ids: Vec<_> = self.dictionary.iter()
...
fn main() {
    ...
    analyzer.save_to_file(dest_path.to_str().unwrap())
